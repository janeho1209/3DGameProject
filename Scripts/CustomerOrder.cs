...
public class CustomerOrder : MonoBehaviour
{
    ...
    public void ReceivePizza(List<IngredientType> deliveredIngredients)
    {
        ...
        if (deliveredIngredients == null)
        {
            Debug.LogError("DELIVERED INGREDIENT LIST IS NULL!");
        }
        else
        {
            Debug.Log("Delivered Count = " + deliveredIngredients.Count);

            Debug.Log("Delivered Ingredients:");
            foreach (var ing in deliveredIngredients)
                Debug.Log("DELIVERED -> " + ing);
        }
    ...
    {
        Debug.Log("ShowEmote called with score: " + score);
        ...
        if (prefab != null)
        {
            ...
            activeBubble.transform.localPosition = Vector3.zero;
            activeBubble.transform.localRotation = Quaternion.identity;

            activeBubble.SetActive(true);

            Debug.Log("Spawned emote: " + prefab.name + " at " + parent.position);
